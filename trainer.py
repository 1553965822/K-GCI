...
class Trainer(object):
    def __init__(self, config):
        ...
        # Load model configuration from best_config.conf
        with open('best_config.conf') as cf:
            best_config = json.load(cf).get(self.config["model"])

        # If model not found, raise an error
        if not best_config:
            raise ValueError(f"Configuration for model {self.config['model']} not found in best_config.conf")

        # Merge the best_config into the config
        for key_, val_ in best_config.items():
            self.config[key_] = val_

        # Ensure that model_name is correctly set
        self.model_name = self.config.get("model_name")

        # Raise an error if model_name is not found or is empty
        if not self.model_name or self.model_name == 'default':
            raise ValueError(f"Model name must be provided in the configuration: {self.config}")
